...
func TestBuildTranslationPrompt(t *testing.T) {
	...
	// Check that the prompt contains the instructions for neutral Spanish
	if !strings.Contains(prompt, "neutral Spanish (español neutro)") {
		t.Errorf("Prompt should specify neutral Spanish translation")
	}
	
	// Check that the prompt contains the lines to translate
	if !strings.Contains(prompt, "Hello, how are you?") || !strings.Contains(prompt, "I'm fine, thank you.") {
	...
	if !strings.Contains(prompt, "Previous context in neutral Spanish") || !strings.Contains(prompt, "Hola, ¿cómo estás?") {
...
func TestProcessTranslatedResponse(t *testing.T) {
	...
	if translatedLines[0].Text[0] != "Hola" || translatedLines[1].Text[0] != "Mundo" {
		t.Errorf("Incorrect translation: %v", translatedLines)
	}
